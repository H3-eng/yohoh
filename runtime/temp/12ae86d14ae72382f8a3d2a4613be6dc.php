<?php /*a:1:{s:76:"/www/users/HA630745/WEB/yahoh/application/admin/view/Newsform/knowledge.html";i:1571630948;}*/ ?>
<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
				...
				<div class="ibox-content">
					<div class="row">
						<div class="deletegroup fl">
							...
							<a button type="button" class="btn btn-success" href="<?php echo url('admin/newsform/knowledgeadd'); ?>">添加文章</a>
						</div>
						<div class="searchgroup fr col-sm-3">
							<div class="input-group">
								...
								<input type="text" placeholder="请输入文章标题" class="input-sm form-control" id="search" autocomplete="off">
...
<script>
	...
	function getData(page){
		$.ajax({
			type:'get',
			url:"<?php echo url('admin/Newsform/knowledgeshow'); ?>",
			data:{page:page},
			dataType:"json",
			success:function(data){
				if(data){
					...
					for(var i=1;i<data.length;i++){
						...
						str+="<td><a  class='fa fa-pencil update' code='"+data[i].id+" 'href='knowledgeupdate/id/"+data[i].id+"'></a>";
	...
	function del(id){
		...
		$("#sure").off().click(function () {
			$("#isdel").hide();
			$.ajax({
				url: "<?php echo url('admin/Newsform/delete'); ?>",
				type: 'post',
				dataType: 'json',
				data: {id:id},
				success:function(data){
	...
	$("#deletesome").off().click(function(){
		...
		$("#sure").off().click(function () {
			...
			$.ajax({
				url: "<?php echo url('admin/Newsform/kdeletesome'); ?>",
				type: 'post',
				dataType: 'json',
				data: {ids:arr},
				success:function(data){
	...
	$("#search").off().keyup(function () {
		...
		$.ajax({
			url: "<?php echo url('admin/Newsform/search'); ?>",
			type: 'post',
			dataType: 'json',
			data: {data:data},
			success:function(data){
		...
		$.ajax({
			...
			success: function (data) {
				if(data){
					...
					str+="<tr><td colspan='8'><a  class='rehome'>返回第一页</a></td></tr>";
					$("#tbody").html(str);
					$("#table-footer").hide();
